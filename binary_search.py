...
def cr_helper1(xs, x):
    ...
    if xs[mid] == x and xs[mid - 1] != x:
        return mid
    if xs[mid] == x and xs[mid - 1] == x:
        return cr_helper1(xs[:mid], x)
    if xs[mid] > x:
        return cr_helper1(xs[mid:], x) + len(xs[:mid])
    if xs[mid] < x:
        return cr_helper1(xs[:mid], x)
...
def cr_helper2(xs, x):
    ...
    if xs[mid] < x and xs[mid - 1] >= x:
        return mid
    if xs[mid] >= x:
        return cr_helper2(xs[mid:], x) + len(xs[:mid])
    if xs[mid] < x:
        return cr_helper2(xs[:mid], x)
...
def count_repeats(xs, x):
    ...
    0
    '''
    return cr_helper2(xs, x) - cr_helper1(xs, x)
...
def find_boundaries(f):
    ...
    return fb_helper(f, -1, 1)

def fb_helper(f, lo, hi):
    mid = ((hi + lo) / 2) + lo
    if f(lo) > f(mid):
        lo *= 2
        fb_helper(f, lo, hi)
